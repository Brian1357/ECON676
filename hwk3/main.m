...
% make BE/ME monthly
BEME_49_mon = ones(1085,49);
BEME_25_mon = ones(1085,25);
BEME_49_mon(7,:) = BEME_49(1,:);
BEME_25_mon(7,:) = BEME_25(1,:);
...
beta_49 = zeros(2,49);
for i = 1:49
    beta_49(:,i) = regress(r_49(:,i),[ones(1085,1) Rm]);
end
beta_49 = beta_49(2,:)';

gamma_49 = zeros(2,1085);
for i = 1:1085
    gamma_49(:,i) = regress((r_49(i,:))',[ones(49,1) beta_49]);
end
gamma_49 = gamma_49';
...
avg_gamma_49 = regress(avg_r_49,[ones(49,1) beta_49]);
...
%Part f
gamma_49_SBM = zeros(4,1079);
for i = 7:1085
    gamma_49_SBM(:,i-6) = regress((r_49(i,:))',[ones(49,1) beta_49 size_49_ln(i-1,:)' BEME_49_ln(i,:)']);
end
gamma_49_SBM = gamma_49_SBM(:,1:end)';

summary_49_SBM = [nanmean(gamma_49_SBM);nanstd(gamma_49_SBM)/sqrt(1079)];
...
beta_25 = zeros(2,25);
for i = 1:25
    beta_25(:,i) = regress(r_25(:,i),[ones(1085,1) Rm]);
end
beta_25 = beta_25(2,:)';

gamma_25 = zeros(2,1085);
for i = 1:1085
    gamma_25(:,i) = regress((r_25(i,:))',[ones(25,1) beta_25]);
end
gamma_25 = gamma_25';
...
avg_gamma_25 = regress(avg_r_25,[ones(25,1) beta_25]);
...
%Part f
gamma_25_SBM = zeros(4,1079);
for i = 7:1085
    gamma_25_SBM(:,i-6) = regress((r_25(i,:))',[ones(25,1) beta_25 size_25_ln(i-1,:)' BEME_25_ln(i,:)']);
end
gamma_25_SBM = gamma_25_SBM(:,1:end)';

summary_25_SBM = [nanmean(gamma_25_SBM);nanstd(gamma_25_SBM)/sqrt(1079)];
...
for i = 1:25
    beta_25_tan(:,i) = regress(r_25(:,i),[ones(1085,1) r_25*TanP_w-rf]);
end
beta_25_tan = beta_25_tan(2,:)';

%part c
avg_gamma_25_tan = regress(avg_r_25,[ones(25,1) beta_25_tan]);
...
beta_25_tan_MY = zeros(2,25);
for i = 1:25
    beta_25_tan_MY(:,i) = regress(r_25(:,i),[ones(1085,1) Rm_MY-rf]);
end
beta_25_tan_MY = beta_25_tan_MY(2,:)';
 
avg_gamma_25_tan_MY = regress(avg_r_25,[ones(25,1) beta_25_tan_MY]);
